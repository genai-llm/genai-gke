...
Copy the content from the Openai py files and paste it on the open notebook and run the same to test.
```bash
cd ~/genai-gke/openai/opeaniappongke
ls
```
copy and paste all files in notebook and run the workload.

also you can build a image with shared docker file and the deploy it as a deployment and expose as service
